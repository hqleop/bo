    ...
    <div class="flex-1 min-h-0 bg-white rounded-lg shadow p-4 flex flex-col">
      ...
      >
        <template #content="{ item }">
          <div v-if="item.value === 'general'" class="flex-1 min-h-0 overflow-auto">
            <div v-if="loading" class="py-8 text-center text-gray-500">
              <UIcon name="i-heroicons-arrow-path" class="animate-spin size-6 mx-auto" />
            </div>
            ...
            <div v-else class="max-w-xl space-y-4">
              ...
              <UFormField label="Основний колір компанії">
                <div class="flex flex-col gap-3">
                  <div class="flex flex-wrap items-center gap-2">
                    <UPopover>
                      <UButton label="Обрати колір" color="neutral" variant="outline">
                        <template #leading>
                          <span :style="colorChipStyle" class="size-3 rounded-full border border-gray-300" />
                        </template>
                      </UButton>

                      <template #content>
                        <UColorPicker v-model="colorDraft" class="p-2" />
                      </template>
                    </UPopover>
                    ...
                    <UButton
                      color="neutral"
                      variant="ghost"
                      :disabled="saving || colorDraft === DEFAULT_COMPANY_PRIMARY_COLOR"
                  ...
                  <p v-if="colorDraft && !normalizedColorDraft" class="text-xs text-red-600">
                    Формат кольору: #RRGGBB
                  </p>
          ...
          <div v-else class="flex-1 min-h-0 flex flex-col gap-4">
            <div class="max-w-xl">
              ...
              <p class="mt-2 text-xs text-gray-500">
                Обрані тут CPV-категорії будуть закріплені за компанією та використовуватимуться для участі в тендерах.
              </p>
            </div>

            <div class="flex-1 min-h-0 flex flex-col">
              <div class="flex items-center justify-between mb-2">
                <h3 class="text-sm font-semibold">Закріплені категорії CPV</h3>
                <UButton
                  size="xs"
                  variant="outline"
                  color="red"
              ...
              <div v-else class="flex-1 min-h-0 overflow-auto">
                <UTable :data="assignedCpvs" :columns="cpvTableColumns" class="w-full">
...
const DEFAULT_COMPANY_PRIMARY_COLOR = "#3b82f6"
...
const company = ref<{ id: number; name: string; edrpou: string; primary_color: string } | null>(null)

const colorDraft = ref(DEFAULT_COMPANY_PRIMARY_COLOR)
const savedCompanyColor = ref(DEFAULT_COMPANY_PRIMARY_COLOR)
...
const normalizedColorDraft = computed(() => normalizeHexColor(colorDraft.value))

const canSaveColor = computed(() => {
  if (!company.value) return false
  return Boolean(normalizedColorDraft.value && normalizedColorDraft.value !== savedCompanyColor.value)
})

const colorChipStyle = computed(() => ({
  backgroundColor: normalizedColorDraft.value || DEFAULT_COMPANY_PRIMARY_COLOR
}))
...
const syncAssignedCpvs = (ids: number[], labels: string[]) => {
  ...
  selectedAssignedIds.value = selectedAssignedIds.value.filter((id) => ids.includes(id))
}
...
const onColorInput = (value: string | number) => {
  colorDraft.value = String(value || "").trim()
}

const resetColorToDefault = () => {
  colorDraft.value = DEFAULT_COMPANY_PRIMARY_COLOR
}
...
const loadData = async () => {
  ...
  try {
    ...
    const normalizedCompanyColor =
      normalizeHexColor(data.primary_color || null) || DEFAULT_COMPANY_PRIMARY_COLOR

    company.value = {
      id: data.id,
      name: data.name,
      edrpou: data.edrpou,
      primary_color: normalizedCompanyColor
    }

    colorDraft.value = normalizedCompanyColor
    savedCompanyColor.value = normalizedCompanyColor
    applyCompanyPrimaryColor(normalizedCompanyColor)
    ...
    assignedCpvs.value = cpvs.map((c) => ({
      id: c.id,
      label: c.label || `${c.cpv_code || ""} - ${c.name_ua || ""}`.trim() || `#${c.id}`
    }))
...
const saveCpvs = async () => {
  ...
  try {
    const { error } = await usersUC.updateCurrentCompanyCpvs({
      cpvIds: assignedCpvs.value.map((c) => c.id)
    })
    if (error) {
...
const savePrimaryColor = async () => {
  if (!company.value) return
  const normalized = normalizedColorDraft.value
  if (!normalized) {
    alert("Невірний формат кольору. Використовуйте #RRGGBB.")
    return
  }
  ...
  try {
    ...
    if (error) {
      alert("Помилка збереження кольору компанії.")
      return
    }

    savedCompanyColor.value = normalized
    if (company.value) company.value.primary_color = normalized
    applyCompanyPrimaryColor(normalized)
    await usersUC.refreshMe()
  } finally {
    saving.value = false
  }
}
