              ...
              <template #content="{ item }">
                        ...
                        >
                          ...
                          <template #warehouse-header>
                            <div class="flex items-center gap-1">
                              <span class="truncate">Місце відвантаження</span>
                              <UButton
                                v-if="!isParticipant"
                                size="xs"
                                color="error"
                ...
                <div v-else-if="item.value === 'criteria'" class="space-y-6">
                  ...
                  <div class="flex gap-4 min-h-0 flex-1">
                    <aside
                      class="w-72 flex-shrink-0 flex flex-col min-h-0 border border-gray-200 rounded-lg bg-white shadow-sm overflow-hidden"
                    >
                      <div class="p-2 border-b border-gray-200">
                        <UInput
          ...
          <div class="space-y-6">
            <div class="rounded-lg p-4 bg-gray-50/50">
              ...
              <div class="flex flex-wrap items-end gap-6">
                ...
                <UFormField label="Вигода по ринковій" class="min-w-[260px]">
                  <USelectMenu
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-3 mt-4">
                ...
                <div class="rounded border border-gray-200 bg-white p-3">
                  <div class="text-xs text-gray-500">Вигода в сумі</div>
                  <div class="text-base font-semibold text-gray-900">
                    {{
...
const carryPreviousTourLoading = ref(false);
...
const canCarryPreviousTourProposals = computed(
  () => Number(tender.value?.tour_number || 1) > 1,
);
...
const decisionMarketMetricHeader = computed(() =>
  decisionMarketMode.value === "current_tour"
    ? "Вигода по орієнтовній ринковій поточного туру"
    : "Вигода по орієнтовній ринковій першого туру",
);
...
const decisionTableRows = computed(() => {
  ...
  return displayTenderPositions.value.map((pos) => {
    ...
    const profitMarketStr =
      profitMarket != null ? formatDecimalDisplay(profitMarket) : "—";
...
const decisionSummary = computed(() => {
  ...
  return {
    bestTotal,
    selectedTotal,
    aggregateTotal: selectedTotal - marketTotal,
  };
});
...
async function loadReferenceCriteria() {
  ...
  try {
    const { data } = await tendersUC.getTenderCriteriaByType("sales");
...
async function carryPreviousTourProposals() {
  ...
  try {
    ...
    carryPreviousTourLoading.value = false;
  }
}
