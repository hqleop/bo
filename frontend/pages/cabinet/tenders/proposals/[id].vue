<template>
  <div>
    ...
    <div v-else class="h-full flex flex-col">
      ...
      <!-- Модальне вікно вибору постачальника -->
      <UModal v-model:open="showSupplierModal">
        <template #content>
          <UCard>
            <template #header>
              <h3 class="text-lg font-semibold">Обрати постачальника</h3>
            </template>
...
definePageMeta({
  layout: "cabinet",
  middleware: "auth",
  meta: { title: "Пропозиції — закупівля" },
});
...
async function goToDecisionStage() {
  ...
  if (data) {
    ...
    await navigateTo(`/cabinet/tenders/${tenderId.value}`);
  }
}
