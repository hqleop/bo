<template>
  <div class="container mx-auto px-40 py-10 max-w-3xl">
    <UCard>
      <template #header>
        <div class="space-y-4">
      ...
      <div v-if="currentStep === 1">
        <UForm :state="step1Form" @submit="onStep1Submit" class="space-y-4s">
          <UFormField label="Прізвище" name="last_name" required>
            <UInput class="w-full" v-model="step1Form.last_name" />
          </UFormField>

          <UFormField label="Ім'я" name="first_name" required>
            <UInput class="w-full" v-model="step1Form.first_name" />
          </UFormField>

          <UFormField label="По батькові" name="middle_name">
            <UInput class="w-full" v-model="step1Form.middle_name" />
          </UFormField>

          <UFormField label="Телефон" name="phone" required>
            <div class="flex">
              <span
                class="inline-flex items-center px-3 rounded-md border border-gray-300 bg-gray-50 text-gray-700 text-sm"
              >
                +380
              </span>
              <UInput
          ...
          <UFormField label="Email" name="email" required>
            <UInput class="w-full" v-model="step1Form.email" type="email" />
          </UFormField>
          ...
          <UButton
            ...
            block
            :loading="loading"
            >Зберегти та продовжити</UButton
          >
        </UForm>
      </div>
      ...
      <div v-else-if="currentStep === 2">
        <div class="space-y-4">
          ...
          >
            <UInput
              class="w-full"
              v-model="step2Form.company_address"
              :disabled="isCompanyDataLocked"
            />
          </UFormField>
          ...
          <UFormField
            v-if="
              step2Form.subject_type === 'individual' &&
              !shouldUseExistingCompanyFlow
            "
      ...
      <div v-else>
        <UForm @submit="onStep3Submit" class="space-y-4">
          <div class="rounded-md border border-gray-200 p-4 space-y-3">
            ...
            <UCheckbox
              v-model="step3Form.goal_tenders"
              name="goal_tenders"
...
const isCompanyDataLocked = computed(() =>
  Boolean(
    companyLookup.value?.exists && companyLookup.value?.has_registered_users,
  ),
);
...
const addressLabel = computed(() => {
  if (
    step2Form.subject_type === "fop_resident" ||
    step2Form.subject_type === "individual"
  ) {
...
const initRegistrationState = async () => {
  ...
  companyId.value =
    Number((me as any)?.registration_company_id ?? null) || null;
...
const loadCountryNumbers = async () => {
  ...
  registrationCountryOptions.value = (data as any[]).map((item) => ({
    ...
    label: String(
      item.label || `${item.number_name || ""} (${item.number_code || ""})`,
    ).trim(),
  }));
};
...
const applyLookupCompanyData = (lookup: RegistrationCompanyLookup) => {
  ...
  cpvSelectedLabels.value = companyCpvs.map((cpv) =>
    String(
      cpv?.label || `${cpv?.cpv_code || ""} - ${cpv?.name_ua || ""}`,
    ).trim(),
  );
...
watch(
  ...
  async (nextSubjectType) => {
    if (
      nextSubjectType === "non_resident" &&
      registrationCountryOptions.value.length === 0
    ) {
...
const openInfoModal = (type: "trade" | "privacy") => {
  if (type === "trade") {
    ...
    infoModalText.value =
      "Погоджуючись, ви приймаєте правила проведення торгів та взаємодії з учасниками на платформі.";
  } else {
    infoModalTitle.value = "Політика конфіденційності";
    infoModalText.value =
      "Погоджуючись, ви приймаєте обробку персональних даних відповідно до політики конфіденційності платформи.";
  }
...
const validateStep2 = () => {
  ...
  const expectedLen = expectedCodeLength(step2Form.subject_type as SubjectType);
  if (expectedLen && code.length !== expectedLen) {
    if (step2Form.subject_type === "legal_resident")
      return "Код ЄДРПОУ має містити 8 цифр.";
    if (step2Form.subject_type === "fop_resident")
      return "ІПН має містити 10 цифр.";
  ...
  if (!shouldUseExistingCompanyFlow.value) {
    ...
    if (
      step2Form.subject_type === "individual" &&
      !step2Form.identity_document
    ) {
      return "Завантажте документ, що підтверджує особу.";
    }
  }

  if (!step2Form.agree_trade_rules)
    return "Потрібно погодитися з регламентом торгів.";
  if (!step2Form.agree_privacy_policy)
    return "Потрібно погодитися з політикою конфіденційності.";
