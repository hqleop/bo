      ...
      <div v-if="currentStep === 2">
        <UForm :state="step2Form" @submit="onStep2Submit" class="space-y-4">
          <UFormGroup label="Тип реєстрації" name="type" required>
            <URadioGroup v-model="step2Form.type" :options="companyTypeOptions" />
          </UFormGroup>
          ...
          <div v-if="step2Form.type === 'new'">
            ...
            <UFormGroup label="Цілі реєстрації" name="goals" required>
              <UCheckbox v-model="step2Form.goal_tenders" label="Проведення тендерів" />
              <UCheckbox v-model="step2Form.goal_participation" label="Участь у тендерах" />
            </UFormGroup>
          </div>

          <div class="flex gap-4">
            <UButton variant="outline" block @click="currentStep = 1">Назад</UButton>
...
const { data: companiesData } = await useFetch(`${config.public.apiBase}/companies/`)
...
const onStep2Submit = async () => {
  ...
  const endpoint = step2Form.type === 'new' ? '/registration/step2/new/' : '/registration/step2/existing/'
  const body = step2Form.type === 'new'
    ? {
