...
const menuLinks = computed(() => {
  ...
  if (permissions.value.includes('dashboard.view')) {
    links.push({ label: 'Загальна аналітика', to: '/cabinet/dashboard', icon: 'i-heroicons-chart-bar' })
  }
  if (permissions.value.includes('tenders.view')) {
    links.push({ label: 'Тендери', to: '/cabinet/tenders', icon: 'i-heroicons-document-text' })
  }
  if (permissions.value.includes('tenders.create')) {
    links.push({ label: 'Створення тендерів', to: '/cabinet/tenders/create', icon: 'i-heroicons-plus-circle' })
  }
  if (permissions.value.includes('participation.view')) {
    links.push({ label: 'Участь в тендерах', to: '/cabinet/participation', icon: 'i-heroicons-hand-raised' })
  }
  if (permissions.value.includes('suppliers.view')) {
    links.push({ label: 'Постачальники', to: '/cabinet/suppliers', icon: 'i-heroicons-building-office' })
  }
  // Довідники
  if (permissions.value.includes('nomenclature.view')) {
    links.push({ label: 'Номенклатури', to: '/cabinet/reference/nomenclature', icon: 'i-heroicons-cube' })
  }
  if (permissions.value.includes('categories.view')) {
    links.push({ label: 'Категорії', to: '/cabinet/reference/categories', icon: 'i-heroicons-folder' })
  }
  if (permissions.value.includes('expenses.view')) {
    links.push({ label: 'Статті витрат', to: '/cabinet/reference/expenses', icon: 'i-heroicons-currency-dollar' })
  }
  if (permissions.value.includes('branches.view')) {
    links.push({ label: 'Філіали підрозділи', to: '/cabinet/reference/branches', icon: 'i-heroicons-building-office-2' })
  }
  if (permissions.value.includes('templates.view')) {
    links.push({ label: 'Шаблони', to: '/cabinet/templates', icon: 'i-heroicons-document-duplicate' })
  }
  if (permissions.value.includes('settings.view')) {
    links.push({ label: 'Налаштування', to: '/cabinet/settings', icon: 'i-heroicons-cog-6-tooth' })
  }
  if (permissions.value.includes('users.manage')) {
    links.push({ label: 'Користувачі', to: '/cabinet/users', icon: 'i-heroicons-users' })
  }
  ...
  if (permissions.value.includes('permissions.manage')) {
    links.push({ label: 'Права доступу', to: '/cabinet/permissions', icon: 'i-heroicons-key' })
  }
  return links
})
